"""
Script to run backtest for PowerX strategy
"""
...
def run_backtest(config_file: str) -> dict:
    """
    ...
    if data_source == 'mt5':
        ...
        data_loader = DataLoader(mt5_broker)
    ...
    if config.get('plot', {}).get('enabled', True):
        fig = engine.plot_results(figsize=(12, 8))
...
def print_stats(stats: dict) -> None:
    ...
    if 'avg_trade_duration' in stats and stats['avg_trade_duration'] is not None:
...
def main():
    ...
    if not os.path.exists(args.config):
        ...
        ConfigManager.create_default_config(args.config, default_config)
